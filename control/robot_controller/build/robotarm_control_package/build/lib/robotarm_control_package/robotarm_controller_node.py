...
class RobotMain(object):
    ...
    def motion_home(self, goal_handle):

        feedback_msg = RobotOrder.Feedback()
        feedback_msg.message = 'motion_home_start'
        ...
        if not self._check_code(code, 'set_servo_angle'):
            return
        print('motion_home_finish')

        feedback_msg.message = 'motion_home_finish'
        goal_handle.publish_feedback(feedback_msg)
    ...
    def motion_place_capsule(self, goal_handle):
        ...
        feedback_msg = RobotOrder.Feedback()
        feedback_msg.message = 'place_capsule_start'
        goal_handle.publish_feedback(feedback_msg)
        ...
        feedback_msg.message = 'place_capsule_finish'
        goal_handle.publish_feedback(feedback_msg)
    
    def motion_grab_cup(self, goal_handle):
    ...
    def motion_topping(self, goal_handle, topping, tray_num):
        ...
        feedback_msg.message = 'topping_finish'
        goal_handle.publish_feedback(feedback_msg)
    
    def motion_make_icecream(self, goal_handle, topping):
        feedback_msg = RobotOrder.Feedback()
        ...
        feedback_msg.message = 'making_icecream_finish'
        goal_handle.publish_feedback(feedback_msg)

    def motion_serve(self, goal_handle, tray_num):
        feedback_msg = RobotOrder.Feedback()
        feedback_msg.message = 'serving_start'
        goal_handle.publish_feedback(feedback_msg)
        ...
        feedback_msg.message = 'serving_finish'
        goal_handle.publish_feedback(feedback_msg)
    
    def motion_trash_capsule(self, goal_handle):
        feedback_msg = RobotOrder.Feedback()
        feedback_msg.message = 'trash_capsule_start'
        goal_handle.publish_feedback(feedback_msg)
...
class RobotActionServer(Node):
    ...
    def execute_callback(self, goal_handle):
        ...
        topping = order_msg.topping_num
        topping_type = order_msg.topping_type

        if order_type == 'icecreaming':
            print('making icecream')
            # --------------icecream start--------------------
            self.robot.motion_home(goal_handle)
            self.robot.motion_grab_capsule(goal_handle, tray_num)
            self.robot.motion_place_capsule(goal_handle)
            self.robot.motion_grab_cup(goal_handle)
            if topping_type == 2:
                self.robot.motion_topping(goal_handle, topping, tray_num)
                self.robot.motion_make_icecream(goal_handle, topping)
            else:
                self.robot.motion_make_icecream(goal_handle, topping)
                self.robot.motion_topping(goal_handle, topping, tray_num)
            self.robot.motion_serve(goal_handle, tray_num)
            self.robot.motion_trash_capsule(goal_handle)
            self.robot.motion_home(goal_handle)
            print('icecream finish')
